var e={};e=import.meta.resolve("iQNoI");var t={};t=import.meta.resolve("2Fo44");var i={};i=import.meta.resolve("T1vHv");let o=[{path:/\/welcome/,component:function(e){let t=document.createElement("div");return t.innerHTML=`
  ...
  `;let i=t.querySelector("timer-game"),o=t.querySelector("img-juego");return i?.addEventListener("timeout",()=>{let t=(c||(c="piedra"),l=function(e,t){let i,o;do{var s;o=t===(s=i=function(){let e=["piedra","papel","tijera"],t=Math.floor(Math.random()*e.length);return e[t]}())?"empate":"piedra"===t&&"tijera"===s||"papel"===t&&"piedra"===s||"tijera"===t&&"papel"===s?"jugador":"cpu"}while("empate"===o)let n={playerMove:t,cpuMove:i,result:o};return{history:[...e.history,n],score:{jugador:e.score.jugador+ +("jugador"===o),cpu:e.score.cpu+ +("cpu"===o),empates:e.score.empates}}}(l,c),c=null,l.history.at(-1));t&&(o?.setAttribute("mode","static"),o?.setAttribute("player",t.playerMove),o?.setAttribute("cpu",t.cpuMove),setTimeout(()=>{e.goTo("/resultado")},1500))}),t},background:!0},{path:/\/resultado/,component:function(e){let t=document.createElement("div"),i=l,o=i.history.at(-1);return o?(t.innerHTML=`
    <style>
      .conteiner {
        width: 100%;
        flex: 1;
...
`;return}this.shadow.innerHTML=`
  ...
  `,this.hideEmptySlots(),this.root.querySelectorAll("slot").forEach(e=>{e.addEventListener("slotchange",()=>this.hideEmptySlots())})}}customElements.get("text-game")||customElements.define("text-game",m);class h extends HTMLElement{constructor(){super(),this.render()}render(){let e=this.attachShadow({mode:"open"}),t=document.createElement("button"),i=document.createElement("style"),o=this.getAttribute("action")??"next";t.className="root",t.textContent=this.textContent??"",t.addEventListener("click",()=>{this.dispatchEvent(new CustomEvent("btn-click",{bubbles:!0,composed:!0,detail:{action:o}}))}),i.innerHTML=`
        .root{
      ...
      `,e.append(t,i)}}customElements.get("btn-component")||customElements.define("btn-component",h);let p=new URL(s).href,g=new URL(n).href;class f extends HTMLElement{shadow;result=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"})}static get observedAttributes(){return["result"]}attributeChangedCallback(e,t,i){"result"===e&&(this.result=i,this.render())}connectedCallback(){this.result=this.getAttribute("result"),this.render()}render(){if(!this.result)return;let e="",t="",i="";"jugador"===this.result&&(e="#4CAF50",t=p,i="Ganaste"),"cpu"===this.result&&(e="#8B5A2B",t=g,i="Perdiste"),"empate"===this.result&&(e="#999",i="Empate"),this.shadow.innerHTML=`
    <style>
      :host {
        display: block;
        width: 100%;
  ...
  `}}customElements.get("result-game")||customElements.define("result-game",f);class b extends HTMLElement{shadow;jugador=0;cpu=0;empates=0;constructor(){super(),this.shadow=this.attachShadow({mode:"open"})}static get observedAttributes(){return["jugador","cpu","empates"]}attributeChangedCallback(e,t,i){let o=Number(i)||0;"jugador"===e&&(this.jugador=o),"cpu"===e&&(this.cpu=o),"empates"===e&&(this.empates=o),this.render()}connectedCallback(){this.jugador=Number(this.getAttribute("jugador"))||0,this.cpu=Number(this.getAttribute("cpu"))||0,this.empates=Number(this.getAttribute("empates"))||0,this.render()}render(){this.shadow.innerHTML=`
      ...
      `}}customElements.get("game-score")||customElements.define("game-score",b);let v=document.querySelector(".root");if(!v)throw Error("No se encontró el contenedor .root");function y(e){history.pushState({},"",e),x(e)}function x(e){for(let t of(console.log("El handleRoute recibio una nueva ruta",e),o))if(t.path.test(e)){t.background?v.classList.add("with-background"):v.classList.remove("with-background");let e=t.component({goTo:y});v.firstChild&&v.firstChild.remove(),v.appendChild(e)}}"/"==location.pathname?y("/welcome"):x(location.pathname),window.onpopstate=function(){x(location.pathname)},document.addEventListener("choice",e=>{console.log("Jugador eligió:",c=e.detail)});
//# sourceMappingURL=12-desafio-piedra-papel-tijeras.50018819.js.map
